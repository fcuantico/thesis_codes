...
# Imported modules
import numpy as np
from morse_potential import Morse_Potential
...
class NAG_SUT_DC_VarT():
    ...
    def __init__(self,r0):
        ...
        pos_minima = 1
        #---------------------------------------------------------------------
        E0=Morse_Potential(r0).morse_energy #<-Initial Energy
        E = [E0]
        #---------------------------------------------------------------------
        F0 = Morse_Potential(r0).force_cart #<-- Initial Force
        F = [F0]                                           #<-- Forces List
        ...
        Nreset = 0
        #---------------------------------------------------------------------
        #alpha = 0.009   #<-- my values of alpha and mu
        #mu = 0.8
        ...
        while k < (Ncycle):
            ...
            #-- FORCE ---------------------------------------------------------
            F[i+1] = Morse_Potential(POS[i+1] + mu*VEL[i+1]).force_cart
            norm_force[i+1] =  np.linalg.norm(F[i+1])
            #-- ENERGY -------------------------------------------------------
            E[i+1]=Morse_Potential(POS[i+1]).morse_energy
