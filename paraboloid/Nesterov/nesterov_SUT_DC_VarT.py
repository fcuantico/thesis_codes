...
# Imported modules
import numpy as np
from paraboloid_potential import Pbloid_Potential
...
class NAG_SUT_DC_VarT():
    ...
    def __init__(self,r0):
        ...
        #---------------------------------------------------------------------
        E0=Pbloid_Potential(r0[0], r0[1]).pbloid_energy #<-Initial Energy
        E = [E0]
        #---------------------------------------------------------------------
        F0 = Pbloid_Potential(r0[0], r0[1]).force_cart #<-- Initial Force
        F = [F0]                                           #<-- Forces List
        ...
        while k < (Ncycle):
            ...
            #-- FORCE ---------------------------------------------------------
            F[i+1] = Pbloid_Potential(POS[i+1][0] + mu*VEL[i+1][0],
                                          POS[i+1][1] +
                                          mu*VEL[i+1][1]).force_cart
            norm_force[i+1] =  np.linalg.norm(F[i+1])
            #-- ENERGY -------------------------------------------------------
            E[i+1]=Pbloid_Potential(POS[i+1][0], POS[i+1][1]).pbloid_energy
