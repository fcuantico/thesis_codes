...
class VV_DC_VarT():
    ...
    def __init__(self,r0,h0):
        ...
        #---------------------------------------------------------------------
        E0=Sphere_Potential (r0[0], r0[1]).sphere_energy     #<-Initial Energy
        E = [E0]
        #---------------------------------------------------------------------
        ...
        while k < (Ncycle):
            ...
            #-- FORCE ---------------------------------------------------------
            F[i+1]=Sphere_Potential(POS[i+1][0], POS[i+1][1]).force_cart
            norm_force[i+1] =  np.linalg.norm(F[i+1])
            #-- VELOCITY ------------------------------------------------------
            VEL[i+1] = VEL[i] + ((F[i]+F[i+1])/2)*h
            #-- ENERGY -------------------------------------------------------
            E[i+1]=Sphere_Potential (POS[i+1][0], POS[i+1][1]).sphere_energy
