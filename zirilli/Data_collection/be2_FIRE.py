...
# Imported modules
import numpy as np
from zirilli_potential import Zirilli_Potential
...
class BE2_FIRE():
    ...
    def __init__(self,r0,h0):
        ...
        MIN = -0.3523                                  #<-- Energy minimum
        pos_minima = np.array([-1.0465, 0])            #<-- Position minimum
        h = h0                                         #<-- Time step
        #---------------------------------------------------------------------
        E0 = Zirilli_Potential(r0[0], r0[1]).zirilli_energy #<-- Initial Energy
        E = [E0]
        #---------------------------------------------------------------------
        F0  = Zirilli_Potential(r0[0], r0[1]).force_cart #<-- Initial Force
        F = [F0]                                           #<-- Forces List
        ...
        while k < (Ncycle):
            ...
            #-- FORCE ---------------------------------------------------------
            F[i+1] = Zirilli_Potential(POS[i+1][0], POS[i+1][1]).force_cart
            norm_force[i+1] =  np.linalg.norm(F[i+1])
            #-- VELOCITY ------------------------------------------------------
            VEL[i+1] = VEL[i] + F[i+1]*h
            #-- ENERGY -------------------------------------------------------
            E[i+1] = Zirilli_Potential(POS[i+1][0], POS[i+1][1]).zirilli_energy
